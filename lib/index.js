...
YandexCheckout.prototype = {
   ...
   * @returns {Promise<Object>}
   */
  _createRefund: function({paymentId, amount, description, idempotenceKey, receipt}) {
   ...
   * }
   */
  createRefund: function({paymentId, amount, description, idempotenceKey, receipt}) {
    var _self = this;
    return this._createRefund({paymentId, amount, description, idempotenceKey, receipt}).then(function(data) {
  ...
  createReceipt(payload, idempotenceKey) {
    return utils.request.call(this, 'POST', 'receipts', payload, idempotenceKey);
  }
};
